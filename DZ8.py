...
def read_all(filename: str) -> str:
    ...
    result = []
    # result = [i for i in range(1,len(list_1) + 1) print(i) ]
    for i in range(1, len(list_1) + 1):
        result.append(f"{i} {list_1[i - 1]}")
    return "\n".join(result)
...
while True:
    ...
    if mode == 1:
        ...
        print(read_all(file))
        num_str = int(input("Выбери строку для переноса: "))
        transfer_data(file, copi, num_str)
